...
def print_word_vectors(word_vectors, write_path):
  ...
  with open(write_path, "w") as f_write:
    for key in word_vectors:
      val = key + " " + " ".join(map(str, numpy.round(word_vectors[key], decimals=6))) + "\n"
      f_write.write(val)
...
def print_all_vectorst(word_vectors, all_vectors, write_path):
  ...
  with open(write_path, "w") as f_write:
    for key in all_vectors:
      ...
      # print(key)
      val = key + " " + " ".join(map(str, numpy.round(all_vectors[key], decimals=6))) + "\n"
      f_write.write(val)
...
def compute_vsp_pairs(word_vectors, vocabulary, rho=0.2):
  ...
  #print(type(word_vectors.values()))
  #exit()
...
def counter_fit(current_experiment, kmulti=1):
  ...
  # if we need to compute the VSP terms.
  if current_experiment.hyper_k3 > 0.0:
