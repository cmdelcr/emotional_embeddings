...
def create_model(input_shape):
	...
	output_classification = Dense(5, activation='sigmoid')(x1)
...
def train_model(model, x_train, y_vad, y_sub):
...
dict_sub = read_subjectivity_clues()
...
embedding_matrix, vocabulary, y_vad, y_sub = filling_embeddings(
			word2idx, word2vec, vocabulary, dict_vad, dict_sub, emb_type, type_matrix_emb)
print('size_embeddings: ', np.shape(embedding_matrix))


print('--------------------------')
print('Creating the model...')
model = create_model(len(embedding_matrix[0]))
model = compile_model(model)

r = train_model(model, embedding_matrix, y_vad, y_sub)
