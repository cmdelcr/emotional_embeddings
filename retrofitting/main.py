...
class ExperimentRun:
	...
	def __init__(self, config_filepath):
		...
		self.pretrained_word_vectors = load_word_vectors(pretrained_vectors_filepath)
		...
		if not os.path.exists('resources/true_emotions.txt'):
			self.create_init_files_emotions()
	...
	def create_init_files_emotions(self):
		print('Loading NRC Emotion Lexicon ...')
		df = pd.read_csv('/home/carolina/corpora/lexicons/NRC-Emotion-Lexicon/NRC-Emotion-Lexicon-Wordlevel-v0.92.txt', 
						keep_default_na=False, header=None, sep='\t')
		values_emo_lex = ['anger', 'disgust', 'sadness', 'surprise', 'fear', 'trust', 'joy', 'anticipation', 'negative', 'positive']
		emotions_emo_lex = ['anger', 'disgust', 'sadness', 'surprise', 'fear', 'trust', 'joy', 'anticipation']
		emotions_polarity = ['negative', 'positive']
		# antonym 
		# anger-fear, disgust-trust, sadness-joy, surprise-anticipation
		dict_data_emo_lex = {}
		dict_true_emotion = []
		dict_opposite_emotion = []
		positive_list = []
		negative_list = []
		...
		for index, row in df.iterrows():
			if str(row[0]) in dict_data_emo_lex:
				...
				arr_val = np.zeros((len(values_emo_lex)))
			arr_val[values_emo_lex.index(str(row[1]))] = int(row[2])
			dict_data_emo_lex[str(row[0])] = arr_val

			if int(row[2]) == 1:
				if str(row[1]) in emotions_emo_lex:
					dict_true_emotion.append((str(row[0]), str(row[1])))
				elif str(row[0]) not in positive_list and str(row[1]) == emotions_polarity[1]:
					positive_list.append(str(row[0]))
				elif str(row[0]) not in negative_list and str(row[1]) == emotions_polarity[0]:
					negative_list.append(str(row[0]))
			else:
				if str(row[1]) in emotions_emo_lex:
					dict_opposite_emotion.append((str(row[0]), str(row[1])))


		self.create_file_true_opposite_emotion(dict_true_emotion, 'true_emotions')
		self.create_file_true_opposite_emotion(dict_opposite_emotion, 'opposite_emotions')
		self.create_polarity_files(positive_list, 'positive')
		self.create_polarity_files(negative_list, 'negative')

		all_words = set(list(dict_data_emo_lex.keys()))
		positive_set = set(positive_list)
		negative_set = set(positive_list)

		rest_words = all_words - positive_set
		rest_words = rest_words - negative_set

		self.create_polarity_files(list(rest_words), 'neutral')
	...
	def create_file_true_opposite_emotion(self, dict_emotion, type_emo):
		with open(type_emo + '.txt', 'w') as file:
			for pais_values in dict_emotion:
				file.write(pais_values[0] + '\t' + pais_values[1] + '\n')

	def create_polarity_files(self, list_polarity, polarity):
		with open(polarity + '.txt', 'w') as file:
			for word in list_polarity:
				file.write(word + '\n')
	...
	def load_experiment_hyperparameters(self):
		...
		print("\nExperiment hyperparameters (k_1, k_2, k_3, delta, gamma, rho):", self.hyper_k1, self.hyper_k2, self.hyper_k3, self.delta, self.gamma, self.rho)
...
def print_word_vectors(word_vectors, write_path):
	...
	with open(write_path, "w") as f_write:
		for key in word_vectors:
			value = key + " " + " ".join(map(str, numpy.round(word_vectors[key], decimals=6))) + "\n"
			f_write.write(value)
...
def simlex_analysis(word_vectors):
	...
	df = pd.read_csv("linguistic_constraints/SimLex-999.txt", sep='\t')
	pair_list = []
	...
	with open("results/simlex_ranking.txt", "w") as f_out_simlex:
	...
	with open("results/counter_ranking.txt", "w") as f_out_counterfitting:
...
def run_experiment(config_filepath):
	...
	print("SimLex score (Spearman's rho coefficient) of initial vectors is:", simlex_analysis(current_experiment.pretrained_word_vectors), "\n")
	
	transformed_word_vectors = counter_fit(current_experiment)
	
	print("\nSimLex score (Spearman's rho coefficient) the counter-fitted vectors is:", simlex_analysis(transformed_word_vectors), "\n")
